...
import { Transaction, CreditCard, FinanceState, VaultDestinationType, RecurringExpense } from '@/types/finance';
import { generateInstallmentTransactions, generateId, getCurrentMonth, calculateUsedLimit } from '@/lib/financeUtils';
import { useToast } from '@/hooks/use-toast';
import { useAuth } from '@/hooks/useAuth';
import { z } from 'zod';

// Validation schemas (mantidos iguais)
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Não jogamos erro no recurringRes para não quebrar o app se a tabela ainda não existir direito
        ...
        const recurringExpenses = (recurringRes.data || []).map((db: DbRecurringExpense) => ({
          id: db.id,
          name: db.name,
          baseAmount: Number(db.base_amount),
